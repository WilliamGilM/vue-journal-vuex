...
describe('Pruebas en el Login Component', () => {
  ...
  const store = createVuexStore({
      status: 'not-authenticated', // 'authenticated', 'no-authenticated', 'authenticating'
      user: null,
      idToken: null,
      refreshToken: null
  })
  ...
  test('debe de redirigir a la ruta no-entry', async() => {
    ...
    await txtEmail.setValue('william@gmail.com')
    await txtPassword.setValue('123456')

    await wrapper.find('form').trigger('submit')

    expect(store.dispatch).toHaveBeenCalledWith('auth/signInUser', { email: 'william@gmail.com', password: '123456'})
